 ...
 *
 * @param context Android context for accessing SharedPreferences and BaseUrlProvider (should be application context)
...
class SecurityServiceImpl(
    ...
    private val client: OkHttpClient = sharedClient.newBuilder()
        .addInterceptor(SecurityServiceRetryInterceptor(context.applicationContext, maxRetries, retryDelayMs))
        .build()
    
    private fun authToken(): String = SharedPreferencesHelper(context).token.toString()
    private fun userUUID(): String = SharedPreferencesHelper(context).userUUID
    ...
    override fun getACLEnvelope(
        ...
        Log.d("SecurityService", "ION-2 - Starting ACL fetch for lock $lockMac")
        
        // Validate inputs
        if (lockMac.isBlank()) {
            Log.e("SecurityService", "ION-2 - Invalid lockMac: empty or blank")
            callback.onFailure(IllegalArgumentException("Lock MAC address cannot be empty"))
            return
        }
        
        if (phoneKeyId <= 0) {
            Log.e("SecurityService", "ION-2 - Invalid phoneKeyId: $phoneKeyId")
        ...
        val token = authToken()
        if (token.isBlank()) {
            Log.e("SecurityService", "ION-2 - Authentication token is not available")
        ...
        try {
            ...
            Log.d("SecurityService", "ION-2 - ACL request body: $body")
            ...
            val baseUrl = BaseUrlProvider.getBaseUrl(context)
            ...
            Log.d("SecurityService", "ION-2 - acl envelope url: ${baseUrl}nse-phone-acl/api/v1/acls")

            client.newCall(request).enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    Log.e("SecurityService", "ION-2 - ACL network failure for lock $lockMac: ${e.message}", e)
                    ...
                    response.use {
                        try {
                            if (!it.isSuccessful) {
                                ...
                                Log.e("SecurityService", "ION-2 - ACL HTTP ${it.code()} for lock $lockMac: $errorMessage")
                                Log.e("SecurityService", "ION-2 - ACL error body: $errorBody")
                                callback.onFailure(
                            ...
                            if (jsonString.isNullOrBlank()) {
                                Log.e("SecurityService", "ION-2 - ACL response body is empty for lock $lockMac")
                                callback.onFailure(IOException("Empty response from server"))
                                return
                            }

                            val root = JSONObject(jsonString)
                            Log.d("SecurityService", "ION-2 - ACL response received for lock $lockMac")

                            // Validate required fields exist
                            if (!root.has("acl")) {
                                Log.e("SecurityService", "ION-2 - ACL object missing in response")
                                callback.onFailure(IOException("Invalid response: missing ACL object"))
                                return
                            }
                            
                            if (!root.has("aclSignature")) {
                                Log.e("SecurityService", "ION-2 - ACL signature missing in response")
                                callback.onFailure(IOException("Invalid response: missing ACL signature"))
                                return
                            }

                            if (!root.has("aclBinary")) {
                                Log.e("SecurityService", "ION-2 - ACL binary missing in response")
                            ...
                            for (field in requiredFields) {
                                if (!aclJson.has(field)) {
                                    Log.e("SecurityService", "ION-2 - ACL missing required field: $field")
                                ...
                                permissions = aclJson.getJSONArray("permissions").let { arr ->
                                    ...
                                    for (i in 0 until arr.length()) {
                                        try {
                                            ...
                                            Log.w("SecurityService", "ION-2 - Unknown permission '${arr.getString(i)}' ignored for lock $lockMac")
                                            unknownCount++
                                        }
                                    }
                                    ...
                                    if (perms.isEmpty() && arr.length() > 0) {
                                        Log.e("SecurityService", "ION-2 - ⚠️ CRITICAL: ACL for lock $lockMac has ${arr.length()} permission(s) but NONE are recognized! " +
                                                "Backend sent: ${(0 until arr.length()).map { arr.getString(it) }}. " +
                                                "This ACL will fail validation and lock cannot be unlocked.")
                                    } else if (unknownCount > 0) {
                                        Log.w("SecurityService", "ION-2 - ACL for lock $lockMac: Recognized ${perms.size} permission(s), ignored $unknownCount unknown permission(s)")
                                    }
                            ...
                            Log.d("SecurityService", "ION-2 - ACL envelope validated for lock $lockMac (expires: ${phoneKeyAcl.expiresAt}) with binary (${aclBinary.length} chars)")
                            callback.onSuccess(phoneKeyAcl, aclSignature, aclBinary)
                        } catch (e: org.json.JSONException) {
                            Log.e("SecurityService", "ION-2 - ACL JSON parsing error for lock $lockMac: ${e.message}", e)
                            callback.onFailure(IOException("Invalid JSON in ACL response: ${e.message}", e))
                        } catch (e: Exception) {
                            Log.e("SecurityService", "ION-2 - ACL processing error for lock $lockMac: ${e.message}", e)
            ...
            Log.e("SecurityService", "ION-2 - Unexpected error fetching ACL for lock $lockMac: ${e.message}", e)
    ...
    override fun getBulkACLEnvelopes(
        ...
        Log.d("SecurityService", "ION-2 - Starting bulk ACL fetch for phoneKeyId $phoneKeyId")
        ...
        try {
            ...
            Log.d("SecurityService", "ION-2 - Bulk ACL request body: $body")
            ...
            Log.d("SecurityService", "ION-2 - Bulk ACL URL: ${baseUrl}nse-phone-acl/api/v1/acls/me")

            client.newCall(request).enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    Log.e("SecurityService", "ION-2 - Bulk ACL network failure: ${e.message}", e)
                    ...
                    response.use {
                        try {
                            if (!it.isSuccessful) {
                                ...
                                Log.e("SecurityService", "ION-2 - Bulk ACL HTTP ${it.code()}: $errorMessage")
                                Log.e("SecurityService", "ION-2 - Bulk ACL error body: $errorBody")
                                callback.onFailure(
                            ...
                            val jsonString = it.body()?.string()
                            if (jsonString.isNullOrBlank()) {
                                Log.e("SecurityService", "ION-2 - Bulk ACL response body is empty")
                                callback.onFailure(IOException("Empty response from server"))
                                return
                            }

                            val root = JSONObject(jsonString)
                            Log.d("SecurityService", "ION-2 - Bulk ACL response received")

                            // Validate response structure
                            if (!root.has("result")) {
                                Log.e("SecurityService", "ION-2 - Bulk ACL response missing 'result' field")
                            ...
                            if (result != "success") {
                                Log.e("SecurityService", "ION-2 - Bulk ACL request failed with result: $result")
                                callback.onFailure(IOException("Bulk ACL request failed: $result"))
                                return
                            }
                            
                            if (!root.has("acls")) {
                                Log.e("SecurityService", "ION-2 - Bulk ACL response missing 'acls' array")
                            ...
                            Log.d("SecurityService", "ION-2 - Processing ${aclsArray.length()} ACLs from bulk response")
                            ...
                            for (i in 0 until aclsArray.length()) {
                                try {
                                    ...
                                    for (field in requiredFields) {
                                        if (!aclObj.has(field)) {
                                            Log.w("SecurityService", "ION-2 - ACL at index $i missing required field: $field, skipping")
                                            continue
                                        }
                                    }
                                    ...
                                    try {
                                        ...
                                        Log.w("SecurityService", "ION-2 - Invalid date format for ACL at index $i (lockMac=$lockMac): ${e.message}, skipping")
                                        continue
                                    }
                                    ...
                                    Log.d("SecurityService", "ION-2 - Parsed bulk ACL for lock $lockMac (expires: $expiresAtEpoch)")
                                } catch (e: Exception) {
                                    Log.w("SecurityService", "ION-2 - Failed to parse ACL at index $i: ${e.message}, skipping")
                                    // Continue processing remaining ACLs
                                }
                            }
                            
                            if (bulkAcls.isEmpty() && aclsArray.length() > 0) {
                                Log.e("SecurityService", "ION-2 - Failed to parse any valid ACLs from ${aclsArray.length()} items")
                                callback.onFailure(IOException("No valid ACLs found in response"))
                                return
                            }
                            
                            Log.d("SecurityService", "ION-2 - Successfully parsed ${bulkAcls.size} bulk ACLs")
                            callback.onSuccess(bulkAcls)
                        } catch (e: org.json.JSONException) {
                            Log.e("SecurityService", "ION-2 - Bulk ACL JSON parsing error: ${e.message}", e)
                            callback.onFailure(IOException("Invalid JSON in bulk ACL response: ${e.message}", e))
                        } catch (e: Exception) {
                            Log.e("SecurityService", "ION-2 - Bulk ACL processing error: ${e.message}", e)
            ...
            Log.e("SecurityService", "ION-2 - Unexpected error fetching bulk ACL: ${e.message}", e)
            callback.onFailure(IOException("Unexpected error: ${e.message}", e))
        }
    }
    ...
    override fun provisionPhone(
        ...
        Log.d("SecurityService", "ION-2 - Starting phone provisioning for user $userID")
        
        // Validate inputs
        if (udid.isBlank()) {
            Log.e("SecurityService", "ION-2 - Invalid udid: empty or blank")
            callback.onFailure(IllegalArgumentException("Device UDID cannot be empty"))
            return
        }
        
        if (publicKey.isBlank()) {
            Log.e("SecurityService", "ION-2 - Invalid publicKey: empty or blank")
            callback.onFailure(IllegalArgumentException("Public key cannot be empty"))
            return
        }
        
        if (userID.isBlank()) {
            Log.e("SecurityService", "ION-2 - Invalid userID: empty or blank")
        ...
        try {
            ...
            Log.d("SecurityService", "ION-2 - Provisioning request body created for user $userID")
            ...
            Log.d("SecurityService", "ION-2 - Provisioning URL: ${baseUrl}nse-provisioning-device/v1/phone-keys")

            client.newCall(request).enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    Log.e("SecurityService", "ION-2 - Provisioning network failure for user $userID: ${e.message}", e)
                    ...
                    response.use {
                        try {
                            if (!it.isSuccessful) {
                                ...
                                Log.e("SecurityService", "ION-2 - Provisioning HTTP ${it.code()} for user $userID: $errorMessage")
                                Log.e("SecurityService", "ION-2 - Provisioning error body: $errorBody")
                                callback.onFailure(
                            ...
                            val jsonString = it.body()?.string()
                            if (jsonString.isNullOrBlank()) {
                                Log.e("SecurityService", "ION-2 - Provisioning response body is empty for user $userID")
                                callback.onFailure(IOException("Empty response from server"))
                                return
                            }
                            ...
                            if (!json.has("key_id")) {
                                Log.e("SecurityService", "ION-2 - Provisioning response missing key_id for user $userID")
                            ...
                            if (keyId <= 0) {
                                Log.e("SecurityService", "ION-2 - Invalid key_id received: $keyId for user $userID")
                                callback.onFailure(IOException("Invalid key_id: $keyId"))
                                return
                            }
                            
                            Log.d("SecurityService", "ION-2 - Phone provisioned successfully for user $userID with keyId=$keyId")
                            callback.onSuccess(keyId)
                        } catch (e: org.json.JSONException) {
                            Log.e("SecurityService", "ION-2 - Provisioning JSON parsing error for user $userID: ${e.message}", e)
                            callback.onFailure(IOException("Invalid JSON in provisioning response: ${e.message}", e))
                        } catch (e: Exception) {
                            Log.e("SecurityService", "ION-2 - Provisioning processing error for user $userID: ${e.message}", e)
            ...
            Log.e("SecurityService", "ION-2 - Unexpected error provisioning phone for user $userID: ${e.message}", e)
            callback.onFailure(IOException("Unexpected error: ${e.message}", e))
        }
    }
}
